// theme.jsx — FXcrypt design tokens, theme application, icon set

// Accent palettes (Binance gold default + alternates)
const ACCENTS = {
  gold:   { main: '#FCD535', deep: '#F0B90B', on: '#0B0E11', glow: 'rgba(252,213,53,0.25)' },
  cyan:   { main: '#00C2FF', deep: '#0097c7', on: '#04121A', glow: 'rgba(0,194,255,0.22)' },
  green:  { main: '#16C784', deep: '#0E9E68', on: '#04130D', glow: 'rgba(22,199,132,0.22)' },
  violet: { main: '#7B61FF', deep: '#5B42D8', on: '#0C0820', glow: 'rgba(123,97,255,0.24)' },
};
...
function buildTheme(dark, accentKey) {
  ...
  if (dark) {
    return {
      '--bg': '#0B0E11', '--bg2': '#12161C', '--surface': '#181A20', '--surface2': '#1E2329',
      '--elevated': '#22272E', '--line': '#2B3139', '--line2': '#363C45',
      '--text': '#EAECEF', '--text2': '#B7BDC6', '--muted': '#848E9C', '--faint': '#5E6673',
      '--accent': a.main, '--accent-deep': a.deep, '--on-accent': a.on, '--glow': a.glow,
      '--up': up, '--down': down, '--up-bg': 'rgba(46,189,133,0.14)', '--down-bg': 'rgba(246,70,93,0.14)',
      '--chip': '#252A31', '--overlay': 'rgba(0,0,0,0.6)', '--shimmer': 'rgba(255,255,255,0.04)',
      '--bar': 'rgba(11,14,17,0.82)',
    };
  }
  return {
    '--bg': '#FFFFFF', '--bg2': '#F8FAFD', '--surface': '#FFFFFF', '--surface2': '#F5F7FA',
    '--elevated': '#FFFFFF', '--line': '#EAECEF', '--line2': '#DDE1E6',
    '--text': '#1E2329', '--text2': '#474D57', '--muted': '#707A8A', '--faint': '#929AA5',
    '--accent': a.main, '--accent-deep': a.deep, '--on-accent': '#1E2329', '--glow': a.glow,
    '--up': '#02A36C', '--down': '#E8324B', '--up-bg': 'rgba(2,163,108,0.10)', '--down-bg': 'rgba(232,50,75,0.10)',
    '--chip': '#F0F2F5', '--overlay': 'rgba(20,24,30,0.4)', '--shimmer': 'rgba(0,0,0,0.03)',
    '--bar': 'rgba(255,255,255,0.86)',
  };
}

function applyTheme(el, dark, accentKey) {
  const vars = buildTheme(dark, accentKey);
  for (const k in vars) el.style.setProperty(k, vars[k]);
  el.style.background = vars['--bg'];
  el.style.color = vars['--text'];
}

// ─── Icons (simple line icons, 24×24, currentColor stroke) ───
const PATHS = {
...
Object.assign(window, { applyTheme, buildTheme, Icon, ACCENTS });
