...
function buildProps(screen: string, app: any, payload: any) {
  const W: any = window;
  switch (screen) {
    case 'pointer': return { go: app.go, layout: app.t.homeLayout, model: app.model, setModel: app.setModel, openChat: app.openChat, user: (W.FX?.user?.name?.split(' ')[0]) || 'there' };
    case 'markets': return { go: app.go };
    case 'wallet': return { go: app.go };
    case 'signals': return { go: app.go, onUpsell: app.onUpsell };
    case 'chat': return { go: app.go, model: app.model, setModel: app.setModel, seed: payload.seed, style: app.t.pointerStyle, onProposalTrade: () => {} };
